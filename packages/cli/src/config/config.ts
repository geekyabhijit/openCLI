...
interface CliArgs {
  ...
  'local-endpoint': string | undefined; // Add local endpoint option
}
...
async function parseArguments(): Promise<CliArgs> {
  const argv = await yargs(hideBin(process.argv))
    .option('model', {
      ...
      default: process.env.GEMINI_MODEL || process.env.LOCAL_MODEL || DEFAULT_QWEN_MODEL, // Use Qwen by default
    })
  ...
  return new Config({
    ...
    approvalMode: (() => {
      const yoloMode = argv.yolo || false ? ApprovalMode.YOLO : 
        // Default to YOLO mode for local models to enable write_file tool
