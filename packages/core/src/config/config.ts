...
export class Config {
  ...
  async refreshAuth(authMethod: AuthType) {
    ...
    console.log('🔧 DEBUG: About to create tool registry');
    this.toolRegistry = await createToolRegistry(this);
    console.log('🔧 DEBUG: Tool registry created');
...
export function createToolRegistry(config: Config): Promise<ToolRegistry> {
  ...
  console.log('🔧 DEBUG: excludeTools:', excludeTools ? Array.from(excludeTools) : 'undefined');

  // helper to create & register core tools that are enabled
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const registerCoreTool = (ToolClass: any, ...args: unknown[]) => {
    const toolName = ToolClass.Name || ToolClass.name;
    const coreToolsCheck = !tools || tools.has(ToolClass.Name) || tools.has(ToolClass.name);
    const excludeToolsCheck = !excludeTools || (!excludeTools.has(ToolClass.Name) && !excludeTools.has(ToolClass.name));
    
    console.log(`🔧 DEBUG: Checking tool ${toolName}: coreTools=${coreToolsCheck}, excludeTools=${excludeToolsCheck}`);
    
    // check both the tool name (.Name) and the class name (.name)
    if (coreToolsCheck && excludeToolsCheck) {
      console.log(`🔧 DEBUG: Registering tool: ${toolName}`);
      registry.registerTool(new ToolClass(...args));
    } else {
      console.log(`🔧 DEBUG: Skipping tool: ${toolName}`);
    }
  };
